...
public class ActionRepositoryImpl {
    ...
    public ArchiveAction saveWithJdbc(ArchiveAction action) {
        if (action.getId() == null) {
            ...
            jdbcTemplate.update(connection -> {
                ...
                ps.setTimestamp(3, Timestamp.valueOf(action.getCreatedAt() != null ?
                    action.getCreatedAt() : LocalDateTime.now()));
                return ps;
            }, keyHolder);
            ...
            jdbcTemplate.update(
                "UPDATE user_actions SET action_type = ?, created_at = ? WHERE id = ?",
                action.getActionType().name(),
                Timestamp.valueOf(action.getCreatedAt()),
                action.getId()
            );
            return action;
        }
    ...
    public List<ArchiveAction> findByResearcherIdWithJdbc(Long researcherId) {
        return jdbcClient.sql(SELECT_BY_RESEARCHER_ID_SQL)
            .param(researcherId)
            .query(archiveActionRowMapper())
            .list();
    }

    public List<ArchiveAction> findByActionTypeWithJdbc(ActionType actionType) {
        return jdbcClient.sql(SELECT_BY_ACTION_TYPE_SQL)
            .param(actionType.name())
            .query(archiveActionRowMapper())
            .list();
    }

    public List<ArchiveAction> findByDateRangeWithJdbc(LocalDateTime startDate, LocalDateTime endDate) {
        return jdbcClient.sql(SELECT_BY_DATE_RANGE_SQL)
            .param(Timestamp.valueOf(startDate))
            .param(Timestamp.valueOf(endDate))
            .query(archiveActionRowMapper())
            .list();
    }

    public List<ArchiveAction> findRecentActionsWithJdbc(int limit) {
        return jdbcClient.sql(SELECT_RECENT_ACTIONS_SQL)
            .param(limit)
            .query(archiveActionRowMapper())
            .list();
    }
    ...
    public long countByResearcherIdWithJdbc(Long researcherId) {
        return jdbcClient.sql(COUNT_BY_RESEARCHER_ID_SQL)
            .param(researcherId)
            .query(Long.class)
            .single();
    }
    ...
    public List<ArchiveAction> findByResearcherIdAndActionTypeWithJdbc(Long researcherId, ActionType actionType) {
        return jdbcClient.sql(SELECT_BY_RESEARCHER_AND_TYPE_SQL)
            .param(researcherId)
            .param(actionType.name())
            .query(archiveActionRowMapper())
            .list();
    ...
    public Map<Long, Long> getCommentActivityLeaderboard(int limit) {
        ...
        return jdbcClient.sql(sql)
            .param(limit)
            .query((rs, rowNum) -> Map.entry(rs.getLong("researcher"), rs.getLong("action_count")))
            .list()
            .stream()
    ...
    public Map<Long, Map<ActionType, Long>> getDetailedActivityLeaderboard(int limit) {
        ...
        List<Map.Entry<Long, Map.Entry<ActionType, Long>>> results = jdbcClient.sql(sql)
            .query((rs, rowNum) -> Map.entry(
                rs.getLong("researcher"),
                Map.entry(ActionType.valueOf(rs.getString("action_type")), rs.getLong("action_count"))
            ))
            .list();
        ...
        return leaderboard.entrySet().stream()
            .sorted(Map.Entry.<Long, Map<ActionType, Long>>comparingByValue((map1, map2) -> {
    ...
    public Map<ActionType, Long> getActionTypeDistribution() {
        String sql = """
            ...
            """;
        return jdbcClient.sql(sql)
            .query((rs, rowNum) -> Map.entry(ActionType.valueOf(rs.getString("action_type")), rs.getLong("action_count")))
            .list()
            .stream()
            .collect(LinkedHashMap::new, (map, entry) -> map.put(entry.getKey(), entry.getValue()), LinkedHashMap::putAll);
    }
    ...
    private RowMapper<ArchiveAction> archiveActionRowMapper() {
        return new RowMapper<ArchiveAction>() {
            @Override
            public ArchiveAction mapRow(ResultSet rs, int rowNum) throws SQLException {
                ArchiveAction action = new ArchiveAction();
                action.setId(rs.getLong("id"));
                action.setActionType(ActionType.valueOf(rs.getString("action_type")));
